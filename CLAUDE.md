...
npm test              # 999 tests  — PASSES with the two database URLs below set; the
...
1. **The database is PostgreSQL with a versioned migration history** since
   Stage 01 (`ADR-0002`). Twenty-five migrations under `prisma/migrations/`; CI applies
...
20. **Every document is a hashed, versioned file** (Stage 09, ADR-0023).
    ...
    it); `DocumentVersion` rows whose bytes are read back hash-verified or
