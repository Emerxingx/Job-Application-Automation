...
| R-16 | ~~Rate limits multiply by instance count~~ **CLOSED as a mechanism in Stage 24** (ADR-0038): `RATE_LIMIT_STORE=postgres` charges one atomic counter per bucket × actor across every instance (exact under concurrency, database test); in-process by default for one instance; `env:check` warns while it is unset. Residual: the operator must set it before the second instance | — | — | — | Done | 24 |
