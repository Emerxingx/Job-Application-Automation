...
| Stripe | IMPLEMENTED-NOT-VALIDATED | Customer email, amounts, metadata. **No card data stored** | Cross-border | Real Checkout + verified webhook. Never run live or in test mode from this codebase (no key in the build environment — CREDENTIAL blocker STRIPE-TEST-KEY). Webhook replay and ordering guarded since Stage 01 (`WebhookEvent`); entitlement consequences of every event through `src/lib/subscription.ts` since Stage 15 (ADR-0030); a refund is recorded and never revokes |
...
| Enterprise SSO — OpenID Connect (Stage 20, ADR-0035) | **IMPLEMENTED-NOT-VALIDATED** | Authorization Code + PKCE, discovery, JWKS verification with jose, JIT provisioning, `requireSso` policy (`src/lib/sso/`). Proven end to end against a LOCAL fake issuer in `tests/enterprise.test.ts`; **no real identity provider (Entra, Okta, Google Workspace) has completed a sign-in**. Needs `SSO_ENCRYPTION_KEY`, a connection recorded at `/console/organizations/:id`, and egress to the issuer |
| SCIM 2.0 provisioning — Users resource (Stage 20, ADR-0035) | **IMPLEMENTED-NOT-VALIDATED** | `/api/scim/v2/Users` (list with `userName eq`, create, get, PATCH/PUT `active` and `name.formatted`, DELETE = deactivate) behind a staff-issued, SHA-256-hashed bearer token scoped to one organisation (`src/lib/scim/`). Exercised against the database; **no real SCIM client has driven it**. Groups, bulk, sorting and ETags are not implemented |
