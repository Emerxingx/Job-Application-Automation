...
| **L-2** | Which Canadian taxonomy datasets (**NOC, TEER, OaSIS, Canadian Skills and Competencies Taxonomy**) may be redistributed within a commercial product, and on what attribution terms? | **OPEN — REQUIRES LEGAL DECISION.** Stage 04 (2026-09-03) built the spine and the gate: every dataset is registered `unrecorded`, the loaders refuse until an admin records the licence + attribution with a reason (audited), and nothing has been ingested. **Stage 04's exit is BLOCKED on this row** for real data; the schema, loaders, crosswalk and classifier are proven on an attributed test fixture | **Founder + IP / data-licensing counsel** | **Stage 04** (Canada occupation & skills taxonomy) — **before any dataset is ingested** | `ADR-0009` keys every occupation on an internal canonical id with jurisdiction codes as attributes, so a dataset that cannot be redistributed is replaced or degraded without a schema change. `SOURCE_ACCESS_POLICY.md` blocks ingestion until the licence is recorded |
...
| **L-4** | What **recruiter / staffing licensing** applies in BC and in each target jurisdiction? | **OPEN — REQUIRES LEGAL DECISION** | **Founder + employment / regulatory counsel** | **Stage 19** (Staffing / Placement OS) | The brief's own requirement — jurisdictional rules as configuration, never hardcoded Canadian globals — is already the Stage 19 design, so the answer populates a rule set rather than altering one. **Stage 19 (2026-09-05):** the rule set exists (`StaffingJurisdictionRule`, ADR-0034): every targeted jurisdiction is a row seeded with a name only; counsel's answer is recorded at `/console/staffing` with a citation, under step-up re-authentication, and audited; a code outside the seeded target list is refused; a placement under an unrecorded jurisdiction is marked unconfirmed and **no invoice is issued there until the rules are recorded** |
| **L-5** | What **consent language** is required for candidate representation by an agency, and for disclosure to an employer? | **OPEN — REQUIRES LEGAL/FOUNDER DECISION** | **Founder + employment / privacy counsel** | **Stage 18** (employer disclosure consent — before any candidate is disclosed to an employer). Agency-representation consent by **Stage 19** | Consent is already modelled as explicit, granular, versioned and revocable, and disclosure is consent-gated by construction (`ROLE_PERMISSION_MATRIX.md`). The open question is the **wording and scope** of a consent record, not whether one exists. **Stage 18 (2026-09-05):** the disclosure consent is built (`employer_disclosure`, ADR-0033) at version `2026-09-05-draft`; `grantConsent` refuses a `-draft` purpose in production, so no candidate is disclosed to an employer in production until the wording is recorded here and the version made final. **Stage 19:** the agency-representation consent is built likewise (`agency_representation`, ADR-0034) at `2026-09-05-draft`, refused in production by the same rule |
| **L-6** | Under what conditions, if any, may the platform **submit an application autonomously** on a candidate's behalf (per source terms, applicable law, consent wording, withdrawal after submission)? | **OPEN — REQUIRES LEGAL + FOUNDER DECISION; NOTHING BUILT** | **Founder + counsel** | Before any Stage 22 design work (`STAGE22_GATE.md` P1, P2, P8, P10) | The three reachable modes stop at the applicant's own click (ADR-0016, ADR-0026); `approved_auto_apply` is refused in code and has no permission row; every credentialed job source is disabled with no recorded legal basis. The gate record lists ten preconditions, every one NOT MET, and the order in which they reopen it. |
