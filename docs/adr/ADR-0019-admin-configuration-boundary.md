...
**Status:** Proposed · **Date:** 2026-09-02 · **First Tier-1 migration done (Stage 03, 2026-09-03):** `PromptRegistry` left the CMS. It is `PromptVersion` in the transactional database, administered at `/console/prompts` (admin role, step-up re-authentication, approval, evaluation-gated promotion, rollback, retirement, an audit row per change). `AtsRulesets` followed in Stage 05 (`AtsRuleset`, `/console/ats-rulesets`: draft → second-admin approval → active with rollback, step-up, audit; the CMS's "heavy stealth" option is gone — ADR-0008). `FieldMappings` followed in Stage 12 (`FieldMappingVersion`, `/console/field-mappings`, the same lifecycle; every application records the version it was prepared with; the CMS collection is removed — ADR-0026). **Every Tier-1 configuration has now left the CMS.** Evidence: `../programme/STAGE03_EVIDENCE.md` §5. **Stage 20 (ADR-0035, 2026-09-05): the remaining admin surface is built on this line** — organisations, tenant policy, platform roles, feature flags (declared in code; `isTierTwoKey` refuses a key naming a security control and a static test keeps every security module free of flag reads), impersonation (read-only), the audit viewer and export, SSO and SCIM administration — every write under step-up with a reason and an audit row. Status: **Proposed, implemented through Stage 20** - the founder's sign-off of the admin capability checklist (STAGE20_EVIDENCE §11) is what accepts it; a stage does not ratify the ADR it builds on.
