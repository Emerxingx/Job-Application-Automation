...
| 24 | Production deployment and readiness | **PASS (engineering) — exit gate NOT MET (no production environment: live, monitored and rollback-rehearsed on the provider are EXTERNAL ACTIONS)** | A scheduler with leased runs (`WorkerRun`) and `npm run worker` for freshness, rollups, retention and case retention; a shared PostgreSQL rate-limit store (R-16 closed as a mechanism; `rateLimit` async at every call site); a per-request script nonce set by the edge gate and proven in the browser over public, candidate, console and CMS pages; `env:check` (no value printed) and the deploy sequence in `DEPLOYMENT.md`; the production smoke suite (in CI); runbooks indexed (`RUNBOOKS.md`: SLOs and alert rules, break-glass with an audited command, support, rollback rehearsed locally); `PRODUCTION_READINESS_GATES.md` re-measured (G4 has no FAIL left); the consolidated release verdict; 20 new tests | PR (draft, stacked on Stage 23), branch `claude/stage-24-production-readiness`. [`STAGE24_EVIDENCE.md`](STAGE24_EVIDENCE.md). ADR-0038 |
