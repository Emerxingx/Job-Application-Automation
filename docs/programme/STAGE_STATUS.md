...
| 17 | Employment services / case-manager companion (Level 0) | **PASS (engineering) — public-sector deployment BLOCKED (LEGAL, L-1)** | Service-provider organisations with supervisor / case-manager / viewer roles as a named set over the ladder; a case exists only with the client's recorded consent (invited → the client accepts under Settings → open; withdrawal revokes); `CaseNote` and `CaseAssessment` RESTRICTED under organisation RLS with every read and write audited first (never a body); what a case manager sees of the client is a delegated audited read after consent; a pure copilot (nine patterns) that writes recommendations and nothing else, the case manager deciding; action plan, referrals to licensed offerings, outcomes with 4/12/24-week follow-ups; per-organisation retention with no purge without a policy; 17 new tests | [PR __PR__](https://github.com/Emerxingx/Job-Application-Automation/pull/__PRN__), branch `claude/stage-17-employment-services` (stacked on Stage 16). [`STAGE17_EVIDENCE.md`](STAGE17_EVIDENCE.md). **No WorkBC integration exists or is claimed** (ADR-0020 Level 0; every page says so); L-1 (public-sector regime, residency, retention) is open, so no public-sector deployment |
