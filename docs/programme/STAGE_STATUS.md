...
| 19 | Staffing / placement OS | **PASS (engineering) — invoicing BLOCKED (LEGAL, L-4); production representation BLOCKED (LEGAL, L-5)** | Staffing agencies with recruiter / delivery / finance / viewer roles as a named set; client contracts, fee structures where the client is the only describable payer, engagements with recruiter ownership; representation as the candidate's per-engagement consent (invited by email, one transaction, read-only under RLS, revocable; draft wording refused in production); placements with a frozen fee, a guarantee period and a stored jurisdiction evaluation; placement invoicing in its own `PL` book with no candidate on it and no relation to candidate billing (proven statically and in the database); guarantee credit; recruiter productivity; jurisdiction rules as staff-recorded data evaluated by a pure engine (an unknown is never a pass; no invoice under an unrecorded jurisdiction); 24 new tests; independent review: 3 HIGH / 9 MEDIUM / 5 LOW, all fixed with tests | [PR #31](https://github.com/Emerxingx/Job-Application-Automation/pull/31), branch `claude/stage-19-staffing-placement` (stacked on Stage 18). [`STAGE19_EVIDENCE.md`](STAGE19_EVIDENCE.md). L-4 and L-5 are open, so no invoice issues and no candidate is represented in production |
| 20 | Enterprise tenant controls, SSO and public-sector readiness | **PASS (engineering, for the controls built) — exit gate PARTIAL (plans/prices, templates, retention, system health still need a deploy; founder checklist unsigned); SSO and SCIM IMPLEMENTED-NOT-VALIDATED** | Staff create verified organisations, suspend, and set tenant policy (require SSO, allowed domains, session ceiling) under step-up; platform roles assigned in the console and defined in code; feature flags declared in code with a Tier-2 refusal (ADR-0019); read-only, reason-required, 60-minute impersonation that `route()` enforces; an audited audit export; OIDC SSO (PKCE, discovery, JWKS, encrypted secret, JIT provisioning) proven against a local issuer; SCIM 2.0 Users behind hashed, organisation-scoped tokens; 37 new tests; independent review: 3 HIGH / 7 MEDIUM / 12 LOW, all but two stated LOWs fixed with tests | [PR #32](https://github.com/Emerxingx/Job-Application-Automation/pull/32), branch `claude/stage-20-enterprise-controls` (stacked on Stage 19). [`STAGE20_EVIDENCE.md`](STAGE20_EVIDENCE.md). ADR-0035 |
| 21 | Advanced reporting and warehouse readiness | **PASS (engineering) — exit gate PARTIAL (no event stream, no scheduler, no warehouse; throughput NOT VERIFIED)** | One platform metric dictionary (founder/platform, financial, AI cost, connector health, career, employer, staffing, case outcomes) mirrored in `METRIC_DICTIONARY.md` with a parity test; a mart registry with RLS scope, writing jobs and a 26-hour SLA; `OrganizationDailyMart` (org-scoped) and `SubscriptionCohortMart` (system) beside the reused `DailyMetric` (platform activity and as-of snapshots) and `DailyRevenueRollup` (payment outcomes, failed amount, invoice count); replace-semantics rollups with a `RollupRun` each, the only readers of the source tables; the console overview and revenue pages, the employer report, staffing productivity and a new supervisor caseload summary read marts only (static test; the two operational queues allow-listed as lists); freshness shown on every mart page and STALE past the SLA; outcomes withheld under five clients; a CSV extraction boundary under `warehouse/<mart>/<day>.csv` with a tested column contract; parity of the mart revenue summary with the live computation, idempotence, RLS scoping, snapshot scope and index use proven against the database; 33 new tests; independent review: 1 HIGH / 9 MEDIUM / 7 LOW, all but one stated LOW fixed with tests | [PR #33](https://github.com/Emerxingx/Job-Application-Automation/pull/33), branch `claude/stage-21-reporting-warehouse` (stacked on Stage 20). [`STAGE21_EVIDENCE.md`](STAGE21_EVIDENCE.md). ADR-0036 |
| 22 | Controlled autonomous application | **BLOCKED BY DESIGN — gate recorded, nothing built** | Deliberately none. [`STAGE22_GATE.md`](STAGE22_GATE.md): what the platform is proven not to do today (three reachable modes, `approved_auto_apply` refused, every provider prepares only, no scheduler, no evasion), ten preconditions each NOT MET (per-source lawfulness in writing, granular revocable consent, hard eligibility gating, grounding as a refusal, per-application audit with reversibility, a Tier-2 kill switch, entitlement volume caps, ToS compliance, a written founder decision, legal review L-6) and the only order in which the gate reopens | [PR #34](https://github.com/Emerxingx/Job-Application-Automation/pull/34), branch `claude/stage-22-autonomous-gate` (stacked on Stage 21). ADR-0016 amended; COMPLIANCE_REGISTER L-6 |
| 23 | Security, performance, accessibility and operational hardening | **PASS (engineering) — exit gate PARTIAL (penetration test EXTERNAL; provider PITR, production performance, monitoring/on-call, nonce CSP, shared rate-limit store open); independent review processed** | One response-header list on every route (CSP without `script-src`, stated); an explicit cross-site refusal on every cookie-bearing write; a public rate-limited `/api/health` that names no host; every unhandled error logged redacted; a secret-hygiene static test and a CycloneDX SBOM per CI run; account erasure as code (fourteen-day grace, scrub in place across every table, files removed, statutory rows kept, audited) with the person's own route and control; `retention:sweep` for every platform-default row and an enforcement column in the matrix; WCAG 2.2 AA (axe) over 43 rendered pages in CI, green after fixing four colour tokens, a combobox, landmarks, labels, target sizes and inline links; performance budgets with `perf:load` and `perf:rollup` measured locally within budget; `db:backup` / `db:restore` rehearsed with the log (twice: the review found the first dump dropped the RLS grants); DR objectives and scenarios; the incident runbook; `PRODUCTION_READINESS_GATES.md` re-measured row by row; review: 3 HIGH / 7 MEDIUM / 11 LOW, all HIGH and MEDIUM fixed; 25 new tests | [PR #35](https://github.com/Emerxingx/Job-Application-Automation/pull/35), branch `claude/stage-23-hardening` (stacked on Stage 22). [`STAGE23_EVIDENCE.md`](STAGE23_EVIDENCE.md). ADR-0037 |
| 24 | Production deployment and readiness | **PASS (engineering) — exit gate NOT MET (no production environment: live, monitored and rollback-rehearsed on the provider are EXTERNAL ACTIONS); independent review processed** | A scheduler with leased runs (`WorkerRun`) and `npm run worker` for freshness, rollups, retention and case retention; a shared PostgreSQL rate-limit store (R-16 closed as a mechanism; `rateLimit` async at every call site); a per-request script nonce set by the edge gate and proven in the browser over public, candidate, console and CMS pages; `env:check` (no value printed) and the deploy sequence in `DEPLOYMENT.md`; the production smoke suite (in CI); runbooks indexed (`RUNBOOKS.md`: SLOs and alert rules, break-glass with an audited command, support, rollback rehearsed locally); `PRODUCTION_READINESS_GATES.md` re-measured (G4 has no FAIL left); the consolidated release verdict; review: 2 HIGH / 10 MEDIUM / 14 LOW, all HIGH and MEDIUM fixed; 23 new tests | [PR #36](https://github.com/Emerxingx/Job-Application-Automation/pull/36), branch `claude/stage-24-production-readiness` (stacked on Stage 23). [`STAGE24_EVIDENCE.md`](STAGE24_EVIDENCE.md). ADR-0038 |
