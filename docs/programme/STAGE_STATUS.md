...
| 18 | Corporate / talent acquisition OS | **PASS (engineering) — production disclosure BLOCKED (LEGAL, L-5)** | Employer organisations with recruiter / hiring manager / interviewer / viewer roles as a named set over the ladder; requisitions published as first-party postings through the connector gate (a canonical `Job`, `source: employer`; closure stated by the requisition); anonymised, scored sourcing honouring recruiter visibility (hidden never sourced; anonymous without identity); disclosure as the candidate's per-employer consent (one record, one transaction; declined is final; revocation withdraws submissions and pool memberships; SELECT-only for the candidate under RLS); a stage machine that cannot pass consent without it; interviews, notes, offers, hires; talent pools of consented candidates; reporting from the organisation's own events; the draft consent wording REFUSED in production by code; 27 new tests | [PR __PR__](__PRURL__), branch `claude/stage-18-talent-acquisition` (stacked on Stage 17). [`STAGE18_EVIDENCE.md`](STAGE18_EVIDENCE.md). L-5 (disclosure consent wording) is open, so no candidate is disclosed to an employer in production |
