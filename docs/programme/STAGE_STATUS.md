...
| 20 | Enterprise tenant controls, SSO and public-sector readiness | **PASS (engineering, for the controls built) — exit gate PARTIAL (plans/prices, templates, retention, system health still need a deploy; founder checklist unsigned); SSO and SCIM IMPLEMENTED-NOT-VALIDATED** | Staff create verified organisations, suspend, and set tenant policy (require SSO, allowed domains, session ceiling) under step-up; platform roles assigned in the console and defined in code; feature flags declared in code with a Tier-2 refusal (ADR-0019); read-only, reason-required, 60-minute impersonation that `route()` enforces; an audited audit export; OIDC SSO (PKCE, discovery, JWKS, encrypted secret, JIT provisioning) proven against a local issuer; SCIM 2.0 Users behind hashed, organisation-scoped tokens; 30 new tests | [PR #32](https://github.com/Emerxingx/Job-Application-Automation/pull/32), branch `claude/stage-20-enterprise-controls` (stacked on Stage 19). [`STAGE20_EVIDENCE.md`](STAGE20_EVIDENCE.md). ADR-0035 |
