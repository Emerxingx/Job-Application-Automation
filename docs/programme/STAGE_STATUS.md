...
| 06 | Job normalisation, deduplication and freshness | **PASS (engineering) — exit PARTIAL** | Canonical job (fifteen derived fields, `canonicalHash`), `JobProvenance` (one job, every source that carries it), dedup measured on a labelled set (precision 1.000 / recall 1.000, 28 pairs), per-source closure never inferred from silence, closed jobs leave the feed, sweep on demand and by command | Branch `claude/stage-06-normalization-dedup-freshness` (stacked on Stage 05), [PR #18](https://github.com/Emerxingx/Job-Application-Automation/pull/18) (draft). [`STAGE06_EVIDENCE.md`](STAGE06_EVIDENCE.md). **No real traffic measured** (no credentialed source); **no scheduler** (Stage 24) |
