...
| 23 | Security, performance, accessibility and operational hardening | **PASS (engineering) — exit gate PARTIAL (penetration test EXTERNAL; provider PITR, production performance, monitoring/on-call, nonce CSP, shared rate-limit store open)** | One response-header list on every route (CSP without `script-src`, stated); an explicit cross-site refusal on every cookie-bearing write; a public rate-limited `/api/health` that names no host; every unhandled error logged redacted; a secret-hygiene static test and a CycloneDX SBOM per CI run; account erasure as code (fourteen-day grace, scrub in place across every table, files removed, statutory rows kept, audited) with the person's own route and control; `retention:sweep` for every platform-default row and an enforcement column in the matrix; WCAG 2.2 AA (axe) over 43 rendered pages in CI, green after fixing four colour tokens, a combobox, landmarks, labels, target sizes and inline links; performance budgets with `perf:load` and `perf:rollup` measured locally within budget; `db:backup` / `db:restore` rehearsed with the log; DR objectives and scenarios; the incident runbook; `PRODUCTION_READINESS_GATES.md` re-measured row by row; 19 new tests | [PR #35](https://github.com/Emerxingx/Job-Application-Automation/pull/35), branch `claude/stage-23-hardening` (stacked on Stage 22). [`STAGE23_EVIDENCE.md`](STAGE23_EVIDENCE.md). ADR-0037 |
