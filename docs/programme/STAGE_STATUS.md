...
| 14 | Candidate mobile | **PASS (contract) — objective PARTIAL** | The plan's exit gate as worded: a published OpenAPI 3.1 contract for the candidate surface (thirteen operations: profile, recommendations, jobs and detail with the eligibility verdict, match analysis with cited evidence, applications and the whole folder, the two applicant writes, interviews, notifications, analytics summary, ATS lookup), hash-locked and versioned with a change rule, proven against the backend in CI with real keys; nine new v1 routes on the same key auth, scopes, envelope and rate limits | [PR #26](https://github.com/Emerxingx/Job-Application-Automation/pull/26), branch `claude/stage-14-candidate-api-contract` (stacked on Stage 13). [`STAGE14_EVIDENCE.md`](STAGE14_EVIDENCE.md). **The Expo app is NOT built** (no toolchain here; the plan sequences it after the frozen contract); push notifications wait on ADR-0011; no generated client |
