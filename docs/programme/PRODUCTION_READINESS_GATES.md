...
| Runtime config (`PromptRegistry`, `AtsRulesets`, `FieldMappings`) under governed admin with versioning, approval and rollback | governed | `PromptRegistry` (Stage 03) and `AtsRulesets` (Stage 05) governed; `FieldMappings` still in the editorial CMS until Stage 12 | **PARTIAL** |
