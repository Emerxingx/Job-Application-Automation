 ...
 */
export function ApplyThroughPlatform({ jobId, company, requisitionOpen, applied }: { jobId: string; company: string; requisitionOpen: boolean; applied: string | null }) {
  ...
  return (
    <Card className="mt-4 p-5">
      ...
      {applied ? (
        <p className="mt-1 text-xs text-muted">You applied through JobPilot; your application is at stage &ldquo;{applied}&rdquo; in the employer&rsquo;s pipeline. Revoke the disclosure under Settings to withdraw.</p>
      ) : requisitionOpen ? (
        <>
          <p className="mt-1 text-xs text-muted">Applying here shows {company} your name, contact details and résumé - and only them. Your self-identification answers are never shared. You can revoke this under Settings.</p>
